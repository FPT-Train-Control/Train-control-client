...
function getSystemPrompt() {
  ...
  return `Bạn là một trợ lý AI chuyên về hệ thống giám sát tàu hỏa Việt Nam. 
Bạn có quyền truy cập vào dữ liệu tàu hỏa thực tế và có thể giúp phân tích, giải thích các xu hướng, và trả lời câu hỏi từ người dùng.

THÔNG TIN DỮ LIỆU HIỆN TẠI:
- Tổng số chuyến tàu: ${stats.totalTrips}
- Tàu hoạt động: ${stats.trainNames.length > 0 ? stats.trainNames.join(", ") : "Không có"}
- Trạm đang giám sát: ${stats.stations.length > 0 ? stats.stations.join(", ") : "Không có"}
- Vận tốc trung bình: ${stats.avgSpeed.toFixed(2)} km/h
- Vận tốc cao nhất: ${stats.maxSpeed.toFixed(2)} km/h
- Vận tốc thấp nhất: ${stats.minSpeed.toFixed(2)} km/h
- Khoảng thời gian dữ liệu: ${stats.dateRange.from} đến ${stats.dateRange.to}

CÁC ĐIỂM CẦN LƯU Ý:
${stats.insights.join("\n")}

Hãy trả lời bằng tiếng Việt. Hãy chuyên tâm vào việc cung cấp phân tích hữu ích dựa trên dữ liệu tàu hỏa.`;
}
...
function calculateDataStats() {
  ...
  const trainNames = [...new Set(data.map(d => d["Tên Tàu"] || "Unknown"))];
  const stations = [...new Set(data.map(d => d["Trạm"] || "Unknown"))];
  const speeds = data.map(d => parseFloat(d["Vận Tốc"] || 0)).filter(s => !isNaN(s));
  ...
  const minSpeed = speeds.length > 0 ? Math.min(...speeds) * 3.6 : 0;
  
  const dates = data.map(d => d["Ngày Đến"]).filter(d => d);
  const dateRange = {
    from: dates.length > 0 ? dates[0] : "N/A",
    to: dates.length > 0 ? dates[dates.length - 1] : "N/A"
  };

  // Generate insights
  const insights = [];
  if (trainNames.length > 0) {
    insights.push(`• Có ${trainNames.length} tàu hoạt động: ${trainNames.slice(0, 3).join(", ")}${trainNames.length > 3 ? "..." : ""}`);
  }
  if (avgSpeed > 80) {
    insights.push(`• Vận tốc trung bình cao (${avgSpeed.toFixed(1)} km/h)`);
  }
  if (maxSpeed > 150) {
    insights.push(`• Vận tốc tối đa đạt ${maxSpeed.toFixed(1)} km/h - cần chú ý giám sát`);
  }
  if (data.length > 100) {
    insights.push(`• Có ${data.length} bản ghi dữ liệu - khối lượng dữ liệu lớn`);
  }
  ...
  return {
    ...
    minSpeed,
    dateRange,
    insights: insights.length > 0 ? insights : ["Dữ liệu đang được thu thập"]
  };
}

async function initializeWebLLM() {
  const toggle = document.getElementById("webllmToggle");
  const closeBtn = document.getElementById("webllmClose");
  const panel = document.getElementById("webllmPanel");
  const sendBtn = document.getElementById("webllmSendBtn");
  const input = document.getElementById("webllmInput");

  toggle.addEventListener("click", toggleWebLLMPanel);
  closeBtn.addEventListener("click", () => {
    panel.classList.add("hidden");
    toggle.classList.remove("active");
  });

  sendBtn.addEventListener("click", sendWebLLMMessage);
  ...
  try {
    updateWebLLMStatus("loading", "Đang tải thư viện WebLLM...");
    
    // Wait a moment for the library to load
    await new Promise(resolve => setTimeout(resolve, 1000));
    
    if (typeof window.webllm !== "undefined") {
      const MLCEngine = window.webllm.MLCEngine;
      WEBLLM_STATE.engine = new MLCEngine();
      
      // Load model
      updateWebLLMStatus("loading", "Đang tải mô hình Llama-3.2-3B...");
      
      await WEBLLM_STATE.engine.reload(WEBLLM_STATE.model);
      
      WEBLLM_STATE.isReady = true;
      updateWebLLMStatus("ready", "✅ AI Trợ Lý sẵn sàng");
      addWebLLMMessage("assistant", "Xin chào! Tôi là AI trợ lý của hệ thống giám sát tàu hỏa. Hãy hỏi tôi bất kỳ câu hỏi nào về dữ liệu tàu hoặc phân tích.");
    } else {
      updateWebLLMStatus("error", "⚠️ WebLLM không tải được");
      addWebLLMMessage("system", "Lỗi: Không thể tải thư viện WebLLM. Vui lòng tải lại trang.");
    }
  } catch (err) {
    console.error("WebLLM initialization error:", err);
    updateWebLLMStatus("error", `❌ Lỗi: ${err.message}`);
    addWebLLMMessage("system", `Lỗi khởi động: ${err.message}`);
  }
}

function toggleWebLLMPanel() {
  const panel = document.getElementById("webllmPanel");
  const toggle = document.getElementById("webllmToggle");
  ...
  if (!panel.classList.contains("hidden")) {
    document.getElementById("webllmInput").focus();
  }
}

function updateWebLLMStatus(status, message) {
  const statusEl = document.getElementById("webllmStatus");
  statusEl.textContent = message;
  statusEl.className = `webllm-status ${status}`;
}

function addWebLLMMessage(role, content) {
  const messagesEl = document.getElementById("webllmMessages");
  const msgDiv = document.createElement("div");
  msgDiv.className = `webllm-message ${role}`;
...
async function sendWebLLMMessage() {
  if (!WEBLLM_STATE.isReady) {
    addWebLLMMessage("system", "⏳ Mô hình đang được tải. Vui lòng chờ...");
    return;
  }
  ...
  WEBLLM_STATE.isLoading = true;

  // Add user message
  addWebLLMMessage("user", userMessage);
  input.value = "";

  try {
    // Add to conversation history
    WEBLLM_STATE.conversationHistory.push({
      role: "user",
      content: userMessage
    });
    ...
    const messages = [
      ...
      ...WEBLLM_STATE.conversationHistory
    ];

    // Get response from LLM
    updateWebLLMStatus("loading", "Đang xử lý...");
    
    const response = await WEBLLM_STATE.engine.chat.completions.create({
      messages: messages,
      temperature: 0.7,
      max_tokens: 1024,
    });
    ...
    addWebLLMMessage("assistant", assistantMessage);
    
    updateWebLLMStatus("ready", "✅ AI Trợ Lý sẵn sàng");
  } catch (err) {
    console.error("WebLLM error:", err);
    addWebLLMMessage("system", `❌ Lỗi: ${err.message}`);
    updateWebLLMStatus("error", `Lỗi: ${err.message}`);
  } finally {
    // Re-enable input
    input.disabled = false;
    sendBtn.disabled = false;
    WEBLLM_STATE.isLoading = false;
    input.focus();
  }
}
